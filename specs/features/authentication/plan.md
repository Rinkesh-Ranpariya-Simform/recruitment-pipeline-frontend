...
| Addition                        | Purpose                                                                                                                                |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`src/middleware.ts`~~         | **Not built** — the refresh cookie's `Path=/api/auth` puts it out of reach of a frontend route request (spec FE-6).                    |
...
- **Reuses:** the house schema pattern — a zod object plus its `z.infer` type export.
...
**`src/middleware.ts`** — **NOT BUILT** (spec FE-6, revised and approved 2026-09-14)

- The refresh cookie is scoped `Path=/api/auth` by the backend, so the browser never attaches it to a frontend route request. A cookie-presence gate would therefore read "no cookie" for signed-in and signed-out visitors alike, bouncing every authenticated hard reload to `/login`.
- Also relevant, though not the reason: Next 16 deprecated `middleware.ts` in favour of `proxy.ts` (R-1 was right to flag API drift here).
- **Route protection is entirely client-side.** `<RequireAuth>` covers AC-F24 and AC-F36; `LoginForm` covers AC-F25. No app-shell flash results, because the authenticated layout already gates on bootstrap (FE-3.1). The one regression is a failing `/refresh` round trip before a signed-out visitor reaches `/login`.
...
| Cookie name `refresh_token`                    | Backend               | — (no client reader)                      | Nothing — the client never reads the cookie (spec FE-6)                                                          |
...
# 3. Build
npm run build
#    proves: route groups and the server/client boundary
...
**Step 4 is not optional.** The feature's main change is an _absence_, and an absence is the one thing a browser pass cannot confirm — an unreachable `createInterviewer()` wrapper or a stale `/team` route renders nothing and still ships. Grep the source; don't rely on the Network tab.
...
| R-5  | ~~**`middleware.ts` can be mistaken for security.**~~ **Moot — the file is not built** (spec FE-6); the risk went with it.                                                                                                                               | A future contributor removes a real guard believing middleware covers it.                                         | In-file comment stating it has no security value (FE-6.4). `AZ-1` in the spec. The backend re-authorizes every request, and is the _only_ authorization in the system — the client gates nothing by role at all.             |
| R-6  | **In-memory token + hard reload.** Every reload starts with no token, so bootstrap must run before any authenticated render.                                                                                                                             | Login flash or an empty app shell on every refresh.                                                               | `(app)/layout.tsx` gates on bootstrap (AC-F20). No routing gate exists, so the loading state is the only thing standing between a reload and the shell — which is sufficient (AC-F20).                                       |
...
2. **Types + schemas + pure modules** — `types.ts`, `lib/schemas/auth.ts` (`loginSchema` only), `access-token.ts`, `redirect.ts`. No React yet, so the rules behind AC-F08 and AC-F11–F14 are settled in one readable place before any UI exists. _Independent of step 3._
3. **`src/lib/api.ts`.** The interceptor lands **before** anything consumes it, and the existing demo page is reloaded to confirm `ApiStatusCard` still works (R-2, R-3). _Independent of step 2._
...
7. ~~**`src/middleware.ts`**~~ — **skipped** (spec FE-6). AC-F24 and AC-F25 are met client-side instead. Run `npm run build` here anyway — first real exposure to R-1.
...
| AC-F24 — anonymous `/pipeline` → `/login?next=/pipeline`                 | `RequireAuth.tsx`                                    | Logged out, visit `/pipeline` → URL becomes `/login?next=/pipeline`                                                                                                                                                                                                                                                           |
| AC-F25 — authenticated `/login` → redirected away                        | `LoginForm.tsx`                                      | Logged in, visit `/login` → redirected to the role landing route                                                                                                                                                                                                                                                              |
...
| AC-F36 — back button after logout → `/login`                             | `RequireAuth.tsx`                                    | Log out, press Back → `/login`, never a cached authenticated view                                                                                                                                                                                                                                                             |
...
| AC-M02 — session survives hard reload                                    | `AuthProvider.tsx`                                   | Hard-reload `/pipeline` while logged in → session survives, no login-page flash                                                                                                                                                                                                                                               |
